...
// ── Mistral Keys ───────────────────────────────────────────────────────────────
// MISTRAL_KEY_A → fila pública (usuários normais)
// MISTRAL_KEY_B → fila Premium (acesso exclusivo)
const MISTRAL_KEYS = {
...
// ── Sistema de 2 Filas ────────────────────────────────────────────────────────
// Fila "normal"  → usuários comuns      → MISTRAL_KEY_A
// Fila "premium" → usuários Premium     → MISTRAL_KEY_B (chave exclusiva)
...
// Retorna a lane correta conforme isPremium
function getLane(isPremium) {
  return isPremium ? lanes.premium : lanes.normal;
}

// Retorna a posição de um userId na sua lane
...
// Helper: retorna o nome da lane ('normal' ou 'premium') conforme isPremium
...
client.on('interactionCreate', async interaction => {
  ...
  if (commandName === 'criar_servidor') {
    ...
    // Escolhe a lane correta (Premium ou Normal) e mostra o embed de fila imediatamente
  ...
  // ── /info ────────────────────────────────────────────────────────────────────
    ...
    await interaction.reply({ embeds: [new EmbedBuilder()
      ...
      .addFields(
        ...
        { name: `${E.loading} Filas`,          value: `Normal: ${lanes.normal.queue.filter(e=>e.userId).length} aguardando · Premium: ${lanes.premium.queue.filter(e=>e.userId).length} aguardando`, inline: true },
      )
      .setFooter({ text: `Architect ${VERSION}` })
      .setTimestamp()] });
  }
