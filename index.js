...
client.on('interactionCreate', async interaction => {
  ...
  // ── /info ────────────────────────────────────────────────────────────────────
    ...
    await interaction.reply({ embeds: [new EmbedBuilder()
      ...
      .addFields(
        { name: `${E.dev} Dev`,               value: 'Velroc',                                                           inline: true },
        { name: `${E.servidores} Servidores`, value: String(client.guilds.cache.size),                                      inline: true },
        { name: `${E.total} Uptime`,          value: `${Math.floor(uptime / 3600)}h ${Math.floor((uptime % 3600) / 60)}m`, inline: true },
        { name: `${E.bot} Stack`,             value: 'Discord.js v14 + Mistral AI',                                        inline: true },
        { name: `${E.info} Versão`,           value: VERSION,                                                               inline: true },
        { name: `${E.gerando} Filas`,         value: `Normal: ${lanes.normal.queue.filter(e=>e.userId).length} | Premium: ${lanes.premium.queue.filter(e=>e.userId).length} aguardando`, inline: true },
      )
      .setFooter({ text: `Architect ${VERSION}` })
      .setTimestamp()] });
  }
  ...
  // ── /dm ──────────────────────────────────────────────────────────────────────
    ...
    for (const [, g] of client.guilds.cache) {
      try {
        ...
        await owner.send({ embeds: [new EmbedBuilder()
          .setTitle(`${E.velroc}  Mensagem Oficial da Velroc`)
          .setColor(0x9b59b6)
          .setDescription(mensagem)
          .setThumbnail(client.user.displayAvatarURL())
