...
const GROQ_MODEL = 'llama-3.3-70b-versatile'; // Modelo rápido e generoso do Groq

// ── Groq Key Rotator ───────────────────────────────────────────────────────────
// Adicione até 5 chaves no .env: GROQ_KEY_1, GROQ_KEY_2, ..., GROQ_KEY_5
// O sistema rotaciona automaticamente e pula chaves em rate limit
const GROQ_KEYS = [
...
if (GROQ_KEYS.length === 0) {
  console.error('❌ Nenhuma GROQ_KEY encontrada no .env! Defina ao menos GROQ_KEY_1.');
  process.exit(1);
}

// Cooldown por chave: se receber 429, marca ela como bloqueada por X ms
const keyCooldowns = new Map(); // index -> timestamp de liberação

function getAvailableKey() {
  const now = Date.now();
  for (let i = 0; i < GROQ_KEYS.length; i++) {
    const cooldownUntil = keyCooldowns.get(i) || 0;
    if (now >= cooldownUntil) return { key: GROQ_KEYS[i], index: i };
  }
  // Todas em cooldown — pega a que libera primeiro
  let earliest = Infinity, earliestIdx = 0;
  for (let i = 0; i < GROQ_KEYS.length; i++) {
    const t = keyCooldowns.get(i) || 0;
    if (t < earliest) { earliest = t; earliestIdx = i; }
  }
  return { key: GROQ_KEYS[earliestIdx], index: earliestIdx, waitUntil: earliest };
}

function markKeyCooldown(index, seconds = 60) {
  keyCooldowns.set(index, Date.now() + seconds * 1000);
  console.warn(`[GROQ] Chave #${index + 1} em cooldown por ${seconds}s. ${GROQ_KEYS.length - [...keyCooldowns.values()].filter(t => t > Date.now()).length} chave(s) disponível(is).`);
}
...
// ── Groq Queue ─────────────────────────────────────────────────────────────────
// Garante 1 chamada por vez com pequeno delay entre elas
let groqBusy = false;
const groqQueue = [];

async function enqueueGroq(task) {
  return new Promise((resolve, reject) => {
    groqQueue.push({ task, resolve, reject });
    processGroqQueue();
  });
}
...
async function processGroqQueue() {
  ...
  groqBusy = true;
  const { task, resolve, reject } = groqQueue.shift();
...
// ── Groq API ───────────────────────────────────────────────────────────────────
// Rotaciona entre as chaves disponíveis automaticamente ao receber 429
async function callGroqRaw(messages, maxTokens = 8000, retries = 6) {
  for (let attempt = 1; attempt <= retries; attempt++) {
    const { key, index, waitUntil } = getAvailableKey();

    // Se TODAS as chaves estão em cooldown, espera a mais próxima liberar
    if (waitUntil && waitUntil > Date.now()) {
      const waitMs = waitUntil - Date.now();
      console.warn(`[GROQ] Todas as chaves em cooldown. Aguardando ${Math.ceil(waitMs / 1000)}s...`);
    ...
    try {
      ...
      if (res.status === 429) {
        // Tenta ler o header Retry-After, senão usa 60s
        const retryAfter = parseInt(res.headers.get('retry-after') || '60', 10);
        markKeyCooldown(index, retryAfter);
        console.warn(`[GROQ] Chave #${index + 1} recebeu 429. Tentando próxima chave... (tentativa ${attempt}/${retries})`);
        continue; // tenta imediatamente com outra chave
      }
      ...
      // Não conta como tentativa de chave — é erro de rede/parse
      console.error(`[GROQ] Erro na tentativa ${attempt}/${retries} (chave #${index + 1}):`, err.message);
      if (attempt === retries) throw err;
      await new Promise(r => setTimeout(r, attempt * 2000));
    }
  }
  throw new Error('Todas as tentativas falharam. Tente novamente mais tarde.');
}

async function callGroq(messages, maxTokens = 8000) {
  return enqueueGroq(() => callGroqRaw(messages, maxTokens));
}
...
async function generateStructure(prompt, onLog, isPremium = false) {
  ...
  await onLog(E.loading, 'GROQ',     'Conectando à API Groq AI...');
  ...
  const roles = await enqueueGeneration(async () => {
    ...
    const systemMsg = `You are an expert Discord server architect. Your task is to generate a COMPLETE and REALISTIC role hierarchy for a Discord server.
RULES:
...
- Permissions array must only contain values from: ADMINISTRATOR, MANAGE_GUILD, MANAGE_CHANNELS, MANAGE_ROLES, KICK_MEMBERS, BAN_MEMBERS, SEND_MESSAGES, VIEW_CHANNEL, MANAGE_MESSAGES.`;
    ...
    const result = await callGroq([
      ...
      {
        role: 'user',
        content: `Server description: "${prompt}"
...
[
  ...
  {"name": "🛡️ Moderador", "color": "#e74c3c", "hoist": true,  "mentionable": true,  "permissions": ["KICK_MEMBERS","BAN_MEMBERS","MANAGE_MESSAGES"]},
  {"name": "🤖 Bot",        "color": "#95a5a6", "hoist": false, "mentionable": false, "permissions": ["SEND_MESSAGES","VIEW_CHANNEL"]},
  {"name": "🔇 Mutado",     "color": "#7f8c8d", "hoist": false, "mentionable": false, "permissions": []}
]
Generate ALL roles now for the server described. Return only the JSON array.`,
      },
    ]);

    const count = Array.isArray(result) ? result.length : 0;
    await onLog(count > 0 ? E.sucesso : E.erro, 'CARGOS', `${count} cargo(s) gerado(s)`);
    return result;
  });
  ...
  // Extrai nomes dos cargos para usar no prompt de canais
  ...
  const categories = await enqueueGeneration(async () => {
    ...
    const systemMsg   = `You are an expert Discord server architect. Your task is to design a COMPLETE, DETAILED Discord server structure.
RULES:
- Return ONLY a valid JSON array, no markdown, no explanation.
...
- Channel names must follow the format: EMOJI・name-with-hyphens (ex: 📢・anuncios, 🔊・chat-de-voz).
- Category names must follow the format: EMOJI ◆ NAME IN CAPS (ex: 🏛️ ◆ INFORMAÇÕES).
- The allowedRoles array must reference role names from the server's actual role list.
- Every category and channel MUST have allowedRoles populated (never leave it empty []).
- Include categories like: welcome/info, rules, general chat, voice rooms, staff-only, and theme-specific ones.`;
    ...
    const result = await callGroq([
      { role: 'system', content: systemMsg },
      {
        role: 'user',
        content: `Server description: "${prompt}"
Available roles on this server: ${roleNames}

Return a JSON array with this exact structure:
[
  {
    "name": "🏛️ ◆ INFORMAÇÕES",
    "allowedRoles": ["👑 Dono", "🛡️ Moderador", "✅ Membro"],
    "channels": [
      {"name": "📢・anuncios",    "type": "announcement", "topic": "Novidades e atualizações oficiais", "allowedRoles": ["👑 Dono", "🛡️ Moderador", "✅ Membro"]},
      {"name": "📋・regras",      "type": "text",         "topic": "Regras do servidor",                "allowedRoles": ["👑 Dono", "🛡️ Moderador", "✅ Membro"]},
      {"name": "👋・apresentacao","type": "text",         "topic": "Apresente-se aqui",                 "allowedRoles": ["👑 Dono", "🛡️ Moderador", "✅ Membro"]},
      {"name": "❓・faq",         "type": "text",         "topic": "Perguntas frequentes",              "allowedRoles": ["👑 Dono", "🛡️ Moderador", "✅ Membro"]},
      {"name": "📌・links-uteis", "type": "text",         "topic": "Links e recursos úteis",            "allowedRoles": ["👑 Dono", "🛡️ Moderador", "✅ Membro"]}
    ]
  }
]
Generate ALL categories and channels for the server described. Each category MUST have at least ${minChannels} channels. Return only the JSON array.`,
      },
    ]);
    ...
    await onLog(
      totalChannels > 0 ? E.sucesso : E.erro,
      'ESTRUTURA',
  ...
  // Verifica se alguma categoria ficou sem canais e loga aviso
  const emptyCats = categories.filter(c => !c.channels?.length);
  if (emptyCats.length > 0)
    console.warn(`[GENERATE] ${emptyCats.length} categoria(s) sem canais: ${emptyCats.map(c => c.name).join(', ')}`);
  ...
  try {
    ...
    const wResult = await callGroq([
      { role: 'system', content: 'You write short, engaging Discord welcome messages in Brazilian Portuguese. Plain text only, no JSON, no markdown.' },
      { role: 'user',   content: `Write a 3-4 line welcome message for a Discord server: "${prompt}". Make it warm and inviting.` },
    ], 300);
    welcomeMessage = typeof wResult === 'string'
      ? wResult
      : (wResult?.content || wResult?.message || '');
...
function buildAnalysisEmbed(prompt, logs) {
  ...
  return new EmbedBuilder()
    ...
    .setFooter({ text: `Architect ${VERSION} • Powered by Groq AI` })
    .setTimestamp();
}
