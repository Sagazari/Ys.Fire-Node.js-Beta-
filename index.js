...
// ── Generate Structure com logs em tempo real ──────────────────────────────────
...
function buildProgressEmbed(title, info, steps) {
  ...
  return new EmbedBuilder()
    ...
    .setColor(0x9b59b6)
    .addFields(
      { name: '📋 Servidor',   value: info.substring(0, 150) },
      ...
      if (pendingCreate.has(confirmId)) {
        ...
        await interaction.user.send({
          embeds: [new EmbedBuilder()
            ...
            .setColor(0xe74c3c)
            .setDescription(`Sua solicitação${tipo ? ` de template **${tipo}**` : ` em **${interaction.guild?.name}**`} expirou por inatividade.\n\nUse o comando novamente quando quiser.`)
            .setFooter({ text: `Architect ${VERSION}` })],
        }).catch(() => {});
      }
      return;
  ...
  try {
    ...
    await owner.send({
      embeds: [new EmbedBuilder()
        ...
        .addFields(
          { name: '📋 Comandos',  value: 'Use **/help** para ver todos os comandos', inline: false },
          { name: '💾 Backup',    value: 'Use **/backup** para salvar a estrutura', inline: false },
          { name: '🛡️ Proteção', value: 'Use **/proteger ativo:true** para ativar o anti-nuke', inline: false },
          { name: '❓ Suporte',   value: 'Use **/help** para ver todos os comandos disponíveis', inline: false },
        )
...
client.on('interactionCreate', async interaction => {
      ...
      try {
        ...
        await interaction.editReply({
          embeds: [new EmbedBuilder()
            ...
            .setColor(0x2ecc71)
            .setDescription(`O servidor **${interaction.guild.name}** foi recriado com sucesso!`)
            .addFields(
              { name: '👥 Cargos',     value: String(structure.roles?.length || 0),     inline: true },
              { name: '📁 Categorias', value: String(structure.categories?.length || 0), inline: true },
              { name: '💬 Canais',     value: String(structure.categories?.reduce((a, c) => a + (c.channels?.length || 0), 0) || 0), inline: true },
            )
            .setFooter({ text: `Architect ${VERSION} • Create. Protect. Restore.` })
            .setTimestamp()],
          components: [],
  ...
  // ── /deletar ─────────────────────────────────────────────────────────────────
    ...
    else { descricao = '🗑️ TUDO será deletado (canais + cargos).'; acao = 'delete_all'; }
  ...
  // ── /ban ─────────────────────────────────────────────────────────────────────
    ...
    if (!member.permissions.has(PermissionFlagsBits.BanMembers)) return interaction.reply({ content: '❌ Sem permissão!', ephemeral: true });
  ...
  // ── /kick ────────────────────────────────────────────────────────────────────
    ...
    if (!member.permissions.has(PermissionFlagsBits.KickMembers)) return interaction.reply({ content: '❌ Sem permissão!', ephemeral: true });
  ...
  // ── /mute ────────────────────────────────────────────────────────────────────
    ...
    if (!member.permissions.has(PermissionFlagsBits.ModerateMembers)) return interaction.reply({ content: '❌ Sem permissão!', ephemeral: true });
  ...
  // ── /clear ───────────────────────────────────────────────────────────────────
    ...
    if (!member.permissions.has(PermissionFlagsBits.ManageMessages)) return interaction.reply({ content: '❌ Sem permissão!', ephemeral: true });
    try {
      await interaction.deferReply({ ephemeral: true });
