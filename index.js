...
MANDATORY RULES — every violation makes the output invalid:
- Return ONLY a raw valid JSON array. No markdown, no backticks, no explanation.
...
- Role names must NEVER match channel or category names — they are completely separate entities.
- nsfw: false on ALL channels, no exceptions.
- rateLimitPerUser: 5 on general channels, 10 on support/media, 0 on announcements and voice.

CRITICAL — CATEGORY STYLE: Pick exactly ONE style. Use it on EVERY category without exception. NEVER use two different styles.
  Style A: ╭──── EMOJI ✦ NAME  (ex: ╭──── ⛺ ✦ Informações)
  Style B: ➢ NAME IN CAPS  (ex: ➢ CONFIGURAÇÕES)
  Style C: ╭⎯⎯⎯╴ ✦ EMOJI NAME  (ex: ╭⎯⎯⎯╴ ✦ 🍫 Inicio)
If you picked Style A, every single category must use Style A. Same for B and C. No exceptions.

CRITICAL — CHANNEL STYLE: Pick exactly ONE style. Use it on EVERY channel without exception. NEVER use two different styles.
  Style 1: EMOJI┃name  (ex: 💬┃chat-geral)
  Style 2: 「EMOJI」name  (ex: 「💬」chat-geral)
  Style 3: EMOJI╺╸name  (ex: 💬╺╸chat-geral)
No spaces around the separator. Channel name after separator: lowercase-with-hyphens only.
If you picked Style 1, every single channel must use Style 1. Same for 2 and 3. No exceptions.
