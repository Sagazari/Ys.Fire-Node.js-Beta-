...
app.get('/auth/callback', async (req, res) => {
  ...
  try {
    ...
    if (rawText.trim().startsWith('<')) {
      console.error('Discord retornou HTML no OAuth:', rawText.substring(0, 300));
      return res.status(400).send('OAuth error: redirect_uri não confere com o registrado no Discord Developer Portal.');
    }

    const tokenData = JSON.parse(rawText);
    if (!tokenData.access_token) {
      console.error('Sem access_token:', tokenData);
      return res.status(400).send(`Discord OAuth error: ${tokenData.error_description || tokenData.error || 'unknown'}`);
    }
...
client.on('interactionCreate', async interaction => {
  ...
  if (commandName === 'criar_servidor') {
    ...
    try {
      await interaction.editReply({ embeds: [new EmbedBuilder().setTitle('🧠 Gerando estrutura...').setColor(0x9b59b6).setDescription('IA analisando seu prompt...').addFields({ name: '📋 Prompt', value: prompt.substring(0, 200) }).setFooter({ text: `Architect ${VERSION}` }).setTimestamp()] }).catch(() => {});
