...
async function captureStructure(guild) {
  ...
  // Membros e seus cargos (exceto bots e cargos managed)
  ...
  for (const [, member] of members) {
    ...
    const roleNames = member.roles.cache
      .filter(r => r.name !== '@everyone' && !r.managed)
      .map(r => r.name);
    if (roleNames.length > 0) memberRoles.push({ userId: member.user.id, roles: roleNames });
  }
...
async function applyStructure(guild, structure, onStep) {
  ...
  // 8. Restaurar cargos dos membros
  if (structure.memberRoles?.length > 0) {
    await onStep('👤', 'Restaurando cargos dos membros...');
    for (const mr of structure.memberRoles) {
      try {
        const member = await guild.members.fetch(mr.userId).catch(() => null);
        if (!member) continue;
        for (const roleName of mr.roles) {
          const role = createdRoles.get(roleName);
          if (role) await member.roles.add(role).catch(() => {});
        }
      } catch (e) { console.error('MemberRole:', e.message); }
    }
    await onStep('✅', `${structure.memberRoles.length} membro(s) com cargos restaurados`);
  }
...
http.createServer((req, res) => res.end('Architect online')).listen(process.env.PORT || 3000, () => console.log(`✅ HTTP na porta ${process.env.PORT || 3000}`));
