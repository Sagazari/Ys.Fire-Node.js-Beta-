...
5. COMMUNITY HIGHLIGHT ROLES (3-5 roles): VIP, Booster, Veteran, Active Member, etc. Thematic and unique.
6. MEMBER TIER (2-3 roles): Verified member, Newcomer/Pending.
7. SPECIAL ROLES (2-4 roles): Partner, Content Creator, Bot, Muted. Always include a bot role and a muted role.
8. THEMED ROLES (2-5 roles): Deeply specific to the server's theme — make them creative, unique, and realistic.

QUALITY STANDARDS:
- Role names must feel authentic, not generic. Use theme-appropriate terminology.
- Colors must follow a visual hierarchy: ownership = warm gold/yellow, staff = red/orange gradient, mods = blue/teal, community = purple/green, members = grey/white, special = unique accent.
...
Design a world-class, professional role hierarchy for this server. Think deeply about the server's purpose, audience, and culture. Every role must serve a clear function and feel like it was crafted by a real, experienced Discord admin.
...
- Channel names: EMOJI・name-with-hyphens (ex: 📢・anuncios, 🎙️・reunioes-gerais).
- Category names must use ONE single separator style, chosen based on what best fits the server's theme and vibe. Pick exactly one from the list below and apply it to EVERY category — consistency is key:
    Style A: EMOJI ══════ NAME ══════ (ex: 🏛️ ══════ INFORMAÇÕES ══════)
    Style B: ── EMOJI  NAME ── (ex: ── 🛡️  MODERAÇÃO ──)
    Style C: EMOJI ◈ NAME ◈ EMOJI (ex: ⚔️ ◈ COMBATE ◈ ⚔️)
    Style D: 「EMOJI NAME」(ex: 「🎮 GAMING」)
    Style E: EMOJI ▸ NAME (ex: 🌟 ▸ DESTAQUES)
    Style F: ═══ EMOJI NAME ═══ (ex: ═══ 🎭 ENTRETENIMENTO ═══)
- Use ONLY that chosen style for all categories. Never mix styles.
- allowedRoles must reference role names from the provided role list — always populated, never empty.
- Every channel must have a detailed, realistic topic description.

REQUIRED CATEGORY STRUCTURE (adapt deeply to the server theme):
1. INFORMATION HUB: announcements, rules, FAQ, roles-info, partnerships, changelogs.
2. WELCOME/ONBOARDING: welcome, introductions, server-guide, role-selection.
3. STAFF/MANAGEMENT ZONE: staff-only channels, meeting rooms, mod-log, ban-appeals, suggestions-review.
4. MAIN COMMUNITY AREA: general chat, off-topic, media sharing, memes, bot commands.
5. VOICE & ACTIVITY: multiple voice rooms, AFK, music, study/focus rooms.
6. THEME-SPECIFIC AREA 1: Deeply specific to the server's purpose. Be creative and realistic.
7. THEME-SPECIFIC AREA 2: Another distinct themed area with unique channels.
8. THEME-SPECIFIC AREA 3: A third themed area — go deep on the server's niche.
9. SUPPORT/HELPDESK: open tickets, support channels, FAQ bot, known issues.
10. EVENTS & SPECIALS: event announcements, giveaways, tournaments, archives.
11. PREMIUM/VIP LOUNGE (if applicable): exclusive channels for premium members.
12. ARCHIVES/LOGS: archived channels, audit logs, old announcements.

QUALITY STANDARDS:
- Every channel must feel purposeful — no filler channels.
- Channel topics must be detailed and informative (2-3 sentences describing the channel's purpose and rules).
- Voice channels must have creative, themed names (not just "Voice 1", "Voice 2").
- Forum channels should be used for structured discussions (suggestions, bug reports, showcase).
- Stage channels for events, presentations, talks.
- The structure must flow logically from top (information) to bottom (archives).
- Think like a real community manager: what does this community ACTUALLY need?`
...
- Channel names: EMOJI・name-with-hyphens.
...
Design a complete, professional, world-class Discord server structure for this community. Think deeply about the server's purpose, culture, and user journey from newcomer to veteran member. Every category and channel must serve a real function and feel authentically human-crafted.

Use ONE single category separator style throughout the entire server — choose the style that best fits this server's theme and apply it consistently to every category.

Return ONLY a JSON array in this exact format (no markdown, no backticks):
[{"name":"🏛️ ══════ INFORMAÇÕES ══════","allowedRoles":["👑 Proprietário","✅ Membro Verificado"],"channels":[{"name":"📢・anuncios-oficiais","type":"announcement","topic":"Canal oficial de anúncios da equipe. Apenas a staff pode enviar mensagens aqui. Fique atento às novidades importantes do servidor.","allowedRoles":["👑 Proprietário","✅ Membro Verificado"]},{"name":"📋・regras-e-conduta","type":"text","topic":"Leia atentamente todas as regras antes de participar. O descumprimento das normas resultará em punições aplicadas pela moderação.","allowedRoles":["👑 Proprietário","✅ Membro Verificado"]}]}]
...
Generate ALL ${minCats}-14 categories, each with ${minChannels}+ channels. Return only the raw JSON array.`
  ...
  try {
    ...
    const wUser = isPremium
      ? `Write a captivating welcome message for: "${prompt}". It should be 5-7 lines long, include a warm greeting, briefly explain what the server is about, highlight 2-3 key things members should do first (like reading rules, introducing themselves, getting roles), and end with an enthusiastic call-to-action. Make it feel human, genuine, and exciting — not robotic.`
      : `Welcome message for: "${prompt}". 3-4 lines, warm and inviting.`;
    const wMsgs = [
      { role: 'system', content: wSystem },
      { role: 'user',   content: wUser },
    ];
    welcomeMessage = await callMistralText(laneName, wMsgs, isPremium ? 500 : 300);
