...
async function callMistral(messages, maxTokens = 8000, retries = 3) {
  for (let attempt = 1; attempt <= retries; attempt++) {
    try {
      ...
      if (res.status === 429) {
        console.warn(`[MISTRAL] Rate limit — aguardando ${attempt * 3}s...`);
    ...
    try {
      ...
      for (const ch of category.channels || []) {
        try {
          ...
          if (type === ChannelType.GuildText || type === ChannelType.GuildAnnouncement) channelData.topic = ch.topic?.substring(0, 1024) || '';
          const created = await guild.channels.create(channelData).catch(e => { console.error('Canal create:', e.message); return null; });
          if (!created) continue;
          if (ch.isSystemChannel) systemChannelId = created.id;
          if (ch.isAfkChannel) afkChannelId = created.id;
          await onStep('💬', `Canal: **${ch.name}**`);
...
client.once('ready', async () => {
  ...
  const commands = [
    ...
    new SlashCommandBuilder().setName('copilot').setDescription('Ativa o Copilot — IA que monitora e corrige o servidor'),
    new SlashCommandBuilder().setName('copilot-off').setDescription('Desativa o Copilot neste servidor'),
  ].map(c => c.toJSON());
...
client.on('interactionCreate', async interaction => {
  ...
  // ── /copilot ──────────────────────────────────────────────────────────────────
  else if (commandName === 'copilot') {
    if (!member.permissions.has(PermissionFlagsBits.Administrator))
      return interaction.reply({ content: lang.noPermission, ephemeral: true });

    if (copilotGuilds.has(guild.id))
      return interaction.reply({ content: '⚠️ O Copilot já está ativo neste servidor. Use **/copilot-off** para desativar.', ephemeral: true });

    await interaction.reply({ embeds: [new EmbedBuilder()
      .setTitle('🤖  Architect Copilot Ativado!')
      .setColor(0x9b59b6)
      .setDescription('O Copilot está monitorando seu servidor.\n\nAnálises serão realizadas a cada **30 minutos** e problemas reportados automaticamente neste canal.')
      .addFields(
        { name: '🔍 O que monitora', value: '• Permissões de cargos\n• Segurança do servidor\n• Organização de canais\n• Vulnerabilidades', inline: false },
      )
      .setFooter({ text: `Architect ${VERSION} • Use /copilot-off para desativar` })
      .setTimestamp()] });

    // Salva no MongoDB
    await mongoDB.collection('copilot').updateOne(
      { guildId: guild.id },
      { $set: { guildId: guild.id, active: true, activatedAt: new Date().toISOString() } },
      { upsert: true }
    );

    // Inicia análise imediata e intervalo de 30 min
    runCopilot(guild);
    const interval = setInterval(() => runCopilot(guild), 30 * 60 * 1000);
    copilotGuilds.set(guild.id, { interval });
  }

  // ── /copilot-off ──────────────────────────────────────────────────────────────
  else if (commandName === 'copilot-off') {
    if (!member.permissions.has(PermissionFlagsBits.Administrator))
      return interaction.reply({ content: lang.noPermission, ephemeral: true });

    if (!copilotGuilds.has(guild.id)) {
      // Verifica se estava salvo no MongoDB
      const doc = await mongoDB.collection('copilot').findOne({ guildId: guild.id });
      if (!doc?.active)
        return interaction.reply({ content: '⚠️ O Copilot não está ativo neste servidor.', ephemeral: true });
    }

    const data = copilotGuilds.get(guild.id);
    if (data?.interval) clearInterval(data.interval);
    copilotGuilds.delete(guild.id);
    ...
    await interaction.reply({ embeds: [new EmbedBuilder()
      .setTitle('🤖  Copilot Desativado')
      .setColor(0x95a5a6)
      .setDescription('O monitoramento automático foi desativado neste servidor.\n\nUse **/copilot** para reativar quando quiser.')
      .setFooter({ text: `Architect ${VERSION}` })
      .setTimestamp()] });
  }
...
// ── Copilot System ────────────────────────────────────────────────────────────
const copilotGuilds = new Map(); // guildId -> { interval, lastCheck }

async function runCopilot(guild) {
  try {
    // Coleta dados do servidor para análise
    ...
    const serverInfo = {
      ...
      channels: channels ? [...channels.values()].map(c => ({
        name: c.name,
        type: c.type,
      })) : [],
      verificationLevel: guild.verificationLevel,
    ...
    const analysis = await callMistral([
      {
        role: 'system',
        content: `You are an expert Discord server administrator and security analyst called Architect Copilot.
Analyze the Discord server data provided and identify issues like:
- Roles with dangerous permissions given to everyone
- Missing essential channels (rules, welcome, etc.)
  ...
  "issues": [
    {
      ...
      "type": "security|organization|missing|permission",
      "title": "Short title",
      "description": "Clear description of the issue",
      "fix": "What was done to fix it or recommendation"
    }
  ],
  "score": 0-100,
  "summary": "Overall server health summary in Portuguese"
}
If no issues found, return {"issues": [], "score": 100, "summary": "Servidor em ótimas condições!"}`
      },
      {
        role: 'user',
        content: `Analyze this Discord server: ${JSON.stringify(serverInfo)}`
      }
    ], 2000).catch(() => null);

    if (!analysis || !analysis.issues) return;

    // Encontra canal de log ou primeiro canal de texto visível
    const logChannel = guild.channels.cache.find(c =>
      c.type === 2 ? false : // não é voz
    ...
    // Se tiver problemas, reporta
    if (analysis.issues.length > 0) {
      const severityEmoji = { high: '🔴', medium: '🟡', low: '🟢' };
      const issueLines = analysis.issues.slice(0, 5).map(i =>
...
> 💡 ${i.fix}`
      ...
      await logChannel.send({ embeds: [new EmbedBuilder()
        .setTitle('🤖  Architect Copilot — Análise do Servidor')
        .setColor(analysis.score >= 80 ? 0x2ecc71 : analysis.score >= 50 ? 0xf39c12 : 0xe74c3c)
        .setDescription(`**Pontuação de Saúde:** ${analysis.score}/100

${analysis.summary}`)
        .addFields({ name: `⚠️ ${analysis.issues.length} problema(s) encontrado(s)`, value: issueLines.substring(0, 1000) })
        .setFooter({ text: `Architect ${VERSION} • Copilot • Use /copilot-off para desativar` })
        .setTimestamp()] }).catch(() => {});
    }

    console.log(`[COPILOT] ${guild.name} — Score: ${analysis.score} — Issues: ${analysis.issues.length}`);
  } catch (e) {
    console.error(`[COPILOT] Erro em ${guild.name}:`, e.message);
  }
}
...
connectDB().then(() => {
  client.login(process.env.DISCORD_TOKEN)
    .then(() => {
      ...
      // Restaura Copilots ativos após reinício
      setTimeout(async () => {
        try {
          const activeCopilots = await mongoDB.collection('copilot').find({ active: true }).toArray();
          for (const doc of activeCopilots) {
            const g = client.guilds.cache.get(doc.guildId);
            if (!g) continue;
            const interval = setInterval(() => runCopilot(g), 30 * 60 * 1000);
            copilotGuilds.set(doc.guildId, { interval });
            console.log(`[COPILOT] Restaurado: ${g.name}`);
          }
          console.log(`[COPILOT] ${activeCopilots.length} servidor(es) monitorado(s)`);
        } catch (e) { console.error('[COPILOT] Restore error:', e.message); }
      }, 5000);
